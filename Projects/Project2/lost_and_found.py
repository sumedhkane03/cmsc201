...
def print_map(the_map,start_pos):
    ...
    :param the_map: the .map file as a 2d list with dictionaries
    :param start_pos: the coordinates [y,x] of the starting position given in the map
    :return: a 2d list which is the full map to be printed.
    '''
    count=0
    rows=[]
    full_map = []
...
def print_full_map(full_map):
    ...
    :param full_map: 2d list returned by the print map function
    :return: nothing but prints out the full map after each move/update
    '''
    rows = []
...
def remove_prev_pos(full_map):
    ...
    :param full_map: full 2d list of the printable map
    :return: nothing but removes the player character from the map to make sure there are no duplicates
    '''
    for i in range(len(full_map)):
...
def check_wall(full_map,player_move,y,x):
    ...
    :param full_map: full printable map
    :param player_move: the command given by the player (w a s d)
    :param y: y coordinate of current player position
    :param x: x coordinate of current player position
    :return: True if there is a wall or door and false if not
    '''
    if player_move == 'w':
        if full_map[y-1][x] == '*':
            return True
...
def use(full_map,y,x,inventory,the_map):
    ...
    :param full_map: full printble map
    :param y: y coordinate of current player position
    :param x: x coordinate of current player position
    :param inventory: current inventory of the player
    :param the_map: the .map 2d list with dictionaries
    :return: nothing but changes the door to a walkable space as well as secrets into doors.
    '''
    if y+1 < len(full_map):
        if full_map[y+1][x] == 'd':
...
def find_start(the_map):
    ...
    :param the_map: .map 2d list with dictionaries
    :return: starting coordinates as given by the map
    '''
    start_pos = []
    for i in range(len(the_map)):
...
def find_item_coords(the_map):
    ...
    :param the_map: .map 2d list with dictionaries
    :return: coords of all items given in the map
    '''
    items = []
    for i in range(len(the_map)):
        for j in range(len(the_map[i])):
...
def move_player(full_map,items,the_map,start_pos):
    ...
    :param full_map: full printed map 2d list
    :param items: list of coordinates of all items on map
    :param the_map: .map 2d list with dictionaries
    :param start_pos: starting coordinates given in the .map file
    :return: nothing but runs every time to move player
    '''
...
def play_game(the_game_map):
    '''
    :param the_game_map: loaded map file
    :return: nothing but calls move function which runs the whole game.
    '''
